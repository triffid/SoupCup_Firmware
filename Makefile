...
size: $(PROGRAM).elf
	@echo "  SIZE                   Atmega328        Atmega644        Atmega1280       Atmega2560"
	@$(OBJDUMP) -h $^ | perl -MPOSIX -ne '/.(text)\s+([0-9a-f]+)/ && do { $$a += eval "0x$$2" }; END { printf "    FLASH : %5d bytes  (%2d%% of %2dkb)    (%2d%% of %2dkb)    (%2d%% of %3dkb)   (%2d%% of %3dkb)\n", $$a, ceil($$a * 100 / (31 * 1024)), 31, ceil($$a * 100 / (63 * 1024)), 63, ceil($$a * 100 / (127 * 1024)), 127, ceil($$a * 100 / (255 * 1024)), 255 }'
	@$(OBJDUMP) -h $^ | perl -MPOSIX -ne '/.(data|bss)\s+([0-9a-f]+)/ && do { $$a += eval "0x$$2" }; END { printf "    RAM   : %5d bytes  (%2d%% of %2dkb)    (%2d%% of %2dkb)    (%2d%% of %3dkb)   (%2d%% of %3dkb)\n", $$a, ceil($$a * 100 / (2 * 1024)), 2, ceil($$a * 100 / (4 * 1024)), 4, ceil($$a * 100 / (8 * 1024)), 8, ceil($$a * 100 / (8 * 1024)), 8 }'
	@$(OBJDUMP) -h $^ | perl -MPOSIX -ne '/.(eeprom)\s+([0-9a-f]+)/ && do { $$a += eval "0x$$2" }; END { printf "    EEPROM: %5d bytes  (%2d%% of %2dkb)    (%2d%% of %2dkb)    (%2d%% of %3dkb)   (%2d%% of %3dkb)\n", $$a, ceil($$a * 100 / (2 * 1024)), 2, ceil($$a * 100 / (2 * 1024)), 2, ceil($$a * 100 / (4 * 1024)), 4, ceil($$a * 100 / (4 * 1024)), 4 }'
